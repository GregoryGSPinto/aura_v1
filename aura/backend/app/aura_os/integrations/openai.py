"""
OpenAIProvider — Cerebro versatil da Aura.

Usado para: conversas gerais, resumos, tarefas variadas,
quando precisar de boa qualidade com custo menor.

O ModelRouter direciona pra ca por default para conversas
e como fallback do Anthropic.
"""
...
class OpenAIProvider:
    ...
    async def generate(self, prompt: str, task_type: str = "conversation") -> str:
        if not self.api_key or not self._client:
            raise ProviderUnavailableError(
                ...
                details={"provider": "openai", "reason": "missing_api_key"},
            )

        if self._is_circuit_open():
            remaining = self._circuit_open_until - time.time()
            raise ProviderUnavailableError(
                f"OpenAIProvider circuit breaker OPEN. Retry in {remaining:.0f}s.",
                details={"provider": "openai", "consecutive_failures": self._consecutive_failures},
            )
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if not choices:
                    raise ProviderUnavailableError(
                        "A API OpenAI retornou resposta vazia.",
                ...
                _log_call("openai", self.model_name, input_tokens, output_tokens, latency_ms, cost_usd, True)

                if not content.strip():
                    raise ProviderUnavailableError(
                        "A API OpenAI retornou conteudo vazio.",
                        details={"provider": "openai"},
                    )
                ...
                raise ProviderAuthError(
                    "OPENAI_API_KEY invalida. Verifique o .env",
                    details={"provider": "openai"},
                )
                ...
                if attempt < max_retries:
                    ...
                    _log_retry("openai", attempt + 1, delay, "rate_limit")
                    await asyncio.sleep(delay)
                    continue
                self._record_failure()
                raise ProviderRateLimitError(
                    f"Rate limit apos {max_retries} tentativas.",
                    details={"provider": "openai", "reason": str(exc)},
                ) from exc
                ...
                if exc.status_code >= 500:
                    ...
                    if attempt < max_retries:
                        delay = _backoff_delay(attempt)
                        _log_retry("openai", attempt + 1, delay, f"server_{exc.status_code}")
                        await asyncio.sleep(delay)
                        continue
                self._record_failure()
                raise ProviderUnavailableError(
                    f"OpenAI API error {exc.status_code}.",
                    details={"provider": "openai", "status_code": exc.status_code},
                ) from exc
                ...
                if attempt < max_retries:
                    delay = _backoff_delay(attempt)
                    _log_retry("openai", attempt + 1, delay, "connection")
                    await asyncio.sleep(delay)
                    continue
                self._record_failure()
                raise ProviderUnavailableError(
    ...
    def _record_failure(self):
        ...
        if self._consecutive_failures >= self._max_failures:
            ...
            logger.error(
                "[openai] Circuit breaker OPEN after %d consecutive failures. Cooldown %ds.",
    ...
    def _estimate_cost(self, input_tokens: int, output_tokens: int) -> float:
        prices = PRICING.get(self.model_name, PRICING["gpt-4o-mini"])
